...
//Scroll Navigator
function scrollto(i) {  
  window.scrollTo(0,footer_height*i)
  nav_overlay.click()
}
...
const totalHeight = scrollWrap.scrollHeight + footer_height - window.innerHeight;
var scroller_thumb_ht = scroller_thumb.getBoundingClientRect().height;
scroller_parent.style.height = scroller_parent.getBoundingClientRect().height + scroller_thumb_ht +"px";

document.addEventListener('scroll',()=>{
  var progress =((window.pageYOffset - scroller_thumb_ht)/totalHeight)*85;
  
  scroller_thumb.style.top = progress +"%"
  
  footer.style.bottom = `${progress-84}%`;
...
const overlay_text_BGcolors =['#DAD4FF','#91C9B0','#98DFFF',"#EBEBEB","#EDB694", "#DEEDFF", "#90D3DA", "#C2E3FC"]
...
function setProjectGotoLink(projectPageIndex) {
  ...
  goto_btn_anchor.setAttribute("href", `../Project Page/project_page_index.html?active_project_index_param=${projectPageIndex}`)
}
...
// quote_sec.style.marginLeft = logo_width + "px";
...
contact_form.addEventListener('submit', function(event) {
  ...
  emailjs.send(service_id,template_id, sender_Data)
  .then(() => {
    console.log('SUCCESS!');
